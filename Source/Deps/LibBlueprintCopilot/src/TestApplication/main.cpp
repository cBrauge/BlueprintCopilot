...
//     There are also special parameters BlueprintID and NodeID, they are there to allow caching internally, so make sure you choose an ID that cannot clash with something else,
//     you must provide them and make sure that you don't reference one before it is actually created.
//     Don't forget to position the nodes so that the companion can clearly read the blueprint when opening it.
//     Don't also forget to finish by CreateBlueprintPermanently
//     If you add variables with AddVariables, don't forget to call UpdateBlueprint before trying to do other actions like creating the getter node for this variable
//
//     + CreateBlueprint(BlueprintName, BlueprintID) : Creates a blueprint, basically calling FKismetEditorUtilities::CreateBlueprint, is stored internally with BlueprintID and referenced with it for future actions.
//     + AddVariable(BlueprintID, VariableName, PinCategory, DefaultValue) : Adds a member variable variableName to a blueprint, with a value if provided, be careful with the pin category, it must be of type UEdGraphSchema_K2, if you don't know what to put for DefaultValue, just put an empty string
//     + AddVariableGetNode(BlueprintID, PropertyName, NodeID) : Creates a node NodeID in BlueprintID for the propertyName, this is a getter node
//     + AddFunctionNode(BlueprintID, FunctionName, NodeID) : Creates a node NodeID in BlueprintID for the functionName, the functionName is for instance something inside UKismetMathLibrary
//     + AddVariableSetNode(BlueprintID, PropertyName, NodeID) : Creates a node NodeID in BlueprintID for the propertyName, this node serves to set a value to the propertyName
//     + CreateLink(SourceNodeID, SourcePinName, DestinationNodeID, DestinationPinName) : Links SourceNodeID->SourcePinName to DestinationNodeID->DestinationPinName via FindPin and MakeLinkTo
//     + PositionNode(NodeID, x, y) : Position NodeID at x, y coordinate in the event graph, Make sure to leave some space for the default created Nodes, be careful, when outputing the json, x and y must be integers, not strings
...
//     So for instance, if I needed to add an int variable "foo" to "name_of_my_blueprint_guiwbeibweifs1651" with default value 5, it would look like this:
//{ "Type": "AddVariable", "BlueprintID" : "name_of_my_blueprint_guiwbeibweifs1651", "VariableName" : "foo", "PinCategory" : "UEdGraphSchema_K2::PC_Int", "DefaultValue" : "5", "Reason" : "Adding variable foo with default value 5 and type UEdGraphSchema_K2::PC_Int to name_of_my_blueprint"}
//
//[INSTRUCTIONS]
