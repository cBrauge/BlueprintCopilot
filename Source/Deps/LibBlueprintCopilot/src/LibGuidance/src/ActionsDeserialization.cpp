    ...
    std::vector<Action> ConvertResponseToActions(std::string_view response)
    {
        ...
        for (const auto& item : json) {
            ...
            if (type == "CreateBlueprint") {
                ...
                std::cout << "Create Blueprint Name: " << createBlueprint.BlueprintName << " with ID: " << createBlueprint.BlueprintID << std::endl;
                ...
                std::cout << " to " << createLink.DestinationNodeID << "." << createLink.DestinationPinName << std::endl;
                actions.emplace_back(createLink);
            }
            else if (type == "AddVariable") {
                auto addVariable{ item.get<AddVariable>() };
                std::cout << "Add variable: " << addVariable.VariableName << " of type: " << addVariable.PinCategory << " with default value: '" << addVariable.DefaultValue << "' to Blueprint: " << addVariable.BlueprintID << std::endl;
                actions.emplace_back(addVariable);
            }
            else if (type == "AddVariableGetNode") {
                auto addVariableGetNode{ item.get<AddVariableGetNode>() };
                std::cout << "Add variable getter node: " << addVariableGetNode.PropertyName << " which will be nodeID: " << addVariableGetNode.NodeID << " inside BlueprintID: " << addVariableGetNode.BlueprintID << std::endl;
                ...
                std::cout << "Add variable setter node: " << addVariableSetNode.PropertyName << " which will be nodeID: " << addVariableSetNode.NodeID << " inside BlueprintID: " << addVariableSetNode.BlueprintID << std::endl;
                actions.emplace_back(addVariableSetNode);
            }
            else if (type == "AddFunctionNode") {
                auto addFunctionNode{ item.get<AddFunctionNode>() };
                std::cout << "Add function node: " << addFunctionNode.FunctionName << " which will be nodeID: " << addFunctionNode.NodeID << " inside BlueprintID: " << addFunctionNode.BlueprintID << std::endl;
                actions.emplace_back(addFunctionNode);
            }
            else if (type == "PositionNode") {
                auto positionNode{ item.get<PositionNode>() };
                std::cout << "Position nodeID: " << positionNode.NodeID << " in x:y: " << positionNode.x << ":" << positionNode.y << std::endl;
