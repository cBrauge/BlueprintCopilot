...
void FBlueprintCopilotModule::StartupModule()
{
    ...
    OnPostEngineInitDelegateHandle =
        FCoreDelegates::OnPostEngineInit.AddRaw(this, &FBlueprintCopilotModule::OnPostEngineInit);
    ...
    FGlobalTabmanager::Get()
        ->RegisterNomadTabSpawner(BlueprintCopilotTabId,
...
void FBlueprintCopilotModule::OnPostEngineInit()
{
    ...
    // This function is valid only if no Commandlet or game is running. It also requires Slate Application to be
    // initialized.
    if ((IsRunningCommandlet() == false) && (IsRunningGame() == false) && FSlateApplication::IsInitialized())
    {
        ...
        {
            // Create a UICommandList and map editor spawning function to the UICommand of opening
            // BlueprintCopilot Editor.
            TSharedPtr<FUICommandList> Commands = MakeShareable(new FUICommandList());
            Commands->MapAction(FBlueprintCopilotCommands::Get().OpenBlueprintCopilotWindow,
            ...
            // Create a Menu Extender, which adds a button that executes the UICommandList of opening
            // BlueprintCopilot Window.
            ...
            MainMenuExtender->AddMenuExtension(
                ...
                FName(TEXT("General")),
#endif
                EExtensionHook::After, Commands, FMenuExtensionDelegate::CreateLambda([](FMenuBuilder& MenuBuilder) {
                    MenuBuilder.AddMenuEntry(FBlueprintCopilotCommands::Get().OpenBlueprintCopilotWindow, NAME_None,
...
TSharedRef<SDockTab> FBlueprintCopilotModule::SpawnEditor(const FSpawnTabArgs& Args)
{
    ...
    if (CanSpawnEditor())
    {
        ...
        TSharedRef<SDockTab> NewTab =
            SAssignNew(EditorTab, SDockTab).TabRole(ETabRole::NomadTab)[Editor->CreateEditorUI()];
